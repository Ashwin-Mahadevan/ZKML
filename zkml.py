...
def threshold_array(arr):
    ...
    Replace the entries of the array
    with +-1 and 0
    '''
...
# def threshold_array(arr, threshold=0.45):
#     ''' arr is an np array.
#     threshold is between 0 and 1.
#     Replace the entries of the array
#     with +-1 and 0
...
class Model:
    def __init__(self, max_input, max_weight, structure):
        ...
        self.depth = len(self.structure)-1  # number of transition matrices and bias vectors
    ...
    def predict(self, inp):
        ...
        if len(x.shape)==1:
            for i in range(self.depth):
                x = (self.weights[i].dot(x).reshape(-1) + self.biases[i].reshape(-1))
        else:
    ...
    def fit(self,X,y,iterations, verbose=1):
        ...
        for i in range(iterations):
            ...
            for i in range(self.depth):
                ...
                self.biases[i][self.biases[i]<0]=0
...
for val, vec in zip(y_train, y_train_bin):
    vec[val] = mpz(M.max_for_layer[-1]) # //M.normalizer
