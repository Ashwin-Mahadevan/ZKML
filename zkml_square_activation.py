...
def fix_gradients(DW, DB, scaling = 1):
    ...
    for i in range(len(W)): 
        W[i] = threshold_scale(W[i])
        B[i] = threshold_scale(B[i])        
    return W, B


def threshold_scale(arr, threshold = 0.1):
    c = abs_max(arr)
    pos = arr.copy() / c
    ...
    pos[pos<0] = 0
    neg[neg>0] = 0
    pos[pos<threshold*maxi] = 0
    neg[neg>threshold*mini] = 0
    pos[pos>0] = 1
    neg[neg<0] = -1
    res = pos + neg
