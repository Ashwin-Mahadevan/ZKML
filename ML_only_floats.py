...
initialize_new_model = 0
load_model_from_memory = 1
batch_training = 0
improve_cost = 0
improve_accuracy = 0
save_model = 0
...
if load_and_arrange_data:
    ...
    x_train = x_train.reshape(60000,784).transpose() / 255
    x_test = x_test.reshape(10000,784).transpose() / 255
    y_train_bin = np.zeros((60000,10), dtype = float)
    for val, vec in zip(y_train, y_train_bin):
        vec[val] = label_mark
    y_test_bin = np.zeros((10000,10), dtype = float)
